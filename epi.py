...
square = [1,1+1j,1j,-1+1j,-1,-1-1j,-1j,1-1j]
...
"""
        ...
        pts = map(lambda k: N*(random.random()-0.5 + 1j*(random.random()-0.5)),range(0,N))
        pts_upper = []
        pts_lower = []
        for pt in pts:
            if (numpy.imag(pt)>0):
                ...
                pts_lower.append(pt)
        pts_upper.sort(key = lambda x: x.real)
        pts_lower.sort(key = lambda x: -x.real)
    ...
    coefs = map(lambda coef: coef/N, coefs)
    step = 0.01
    ...
    myplots.plot(numpy.real(sampled_f),numpy.imag(sampled_f),numpy.real(pts),numpy.imag(pts),'ro')
    xbounds = myplots.get_xbound()
    ybounds = myplots.get_ybound()
    myplots.axis([xbounds[0],xbounds[1],ybounds[0]*1.1,ybounds[1]*1.5])
    leg = myplots.legend((r'$f(z)$', 'Points'),'upper center', shadow=True)
    myplots.set_title(title)
    pylab.show()      
